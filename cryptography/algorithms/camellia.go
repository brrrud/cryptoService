...
func NewCamellia(key []byte) (*CamelliaCipher, error) {
	klen := len(key)
	switch klen {
	default:
...
func (c *CamelliaCipher) Encrypt(src []byte) ([]byte, error) {
...
func (c *CamelliaCipher) Decrypt(src []byte) ([]byte, error) {
	dst := make([]byte, len(src))
...
func (c *CamelliaCipher) GetBlockSize() int {
	return BlockSize
}
