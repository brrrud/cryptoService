...
func (s *SerpentCipher) Encrypt(plaintext []byte) ([]byte, error) {
	...
	dst := make([]byte, len(plaintext)) // Выделение буфера для зашифрованных данных
	encryptBlock(dst, plaintext, &s.subkeys)
	return dst, nil
}
...
func (s *SerpentCipher) Decrypt(ciphertext []byte) ([]byte, error) {
	...
	dst := make([]byte, len(ciphertext)) // Выделение буфера для расшифрованных данных
	decryptBlock(dst, ciphertext, &s.subkeys)
	return dst, nil
}
